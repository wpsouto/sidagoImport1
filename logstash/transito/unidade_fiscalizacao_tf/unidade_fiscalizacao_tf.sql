SELECT tro.id_termoobjetivo                            AS id,
       ...
       COALESCE(mf.lat, 0)                             AS fiscalizado_municipio_gps_latitude,
       COALESCE(mf.lon, 0)                             AS fiscalizado_municipio_gps_longitude,
...
FROM fisc.termo_fiscalizacao AS tf
         ...
         INNER JOIN rh.pessoa AS pse ON pse.id = tf.id_criadortermo
         INNER JOIN fisc.unidademovel_funcionario AS func ON pse.id = func.id_pessoa
         INNER JOIN fisc.unidademovel AS unid ON func.id_unidademovel = unid.id_unidademovel
         ...
         LEFT JOIN rh.lotacao AS mf_l ON mf.loc_nu = mf_l.id_localidade AND mf_l.id_lotacaotipo = 3 AND mf_l.bo_ativo = true AND mf_l.bo_organograma = true
         LEFT JOIN rh.lotacao AS mf_lr ON mf_lr.id = mf_l.id_lotacao_pai AND mf_lr.id_lotacaotipo = 2 AND mf_lr.bo_ativo = true AND mf_lr.bo_organograma = true

WHERE tf.bo_termoautojuridico = false
    AND tf.ts_alteracao:: date >= current_date - interval '3 DAY'
