SELECT ac.id_aglomeracaociclo                                          AS id,
       ...
       case when COALESCE(aebo.total, 0) > 0 then 'Sim' else 'Não' end AS bovideo_presente,
       case when COALESCE(aee.total, 0) > 0 then 'Sim' else 'Não' end  AS equideo_presente,
       case when COALESCE(aeco.total, 0) > 0 then 'Sim' else 'Não' end AS caprino_presente,
...
FROM agrocomum.inscricaoestadual AS ie
         ...
         INNER JOIN agrocomum.classificacao_macro_composta AS cmc ON cmc.id_classificacaooriginal = c.id_classificacao AND cmc.id_classificacaomacro = 5
         INNER JOIN dne.log_localidade AS ll ON e.id_localidade = ll.loc_nu
         INNER JOIN rh.lotacao AS l ON l.id_localidade = ac.id_localidade AND l.id_lotacaotipo = 3 AND l.bo_ativo = true AND l.bo_organograma = true --Unidade Local
         ...
         LEFT JOIN (SELECT ac.id_aglomeracaociclo              AS id,
                    ...
                    FROM gta.gta_destino AS gt
                             ...
                             INNER JOIN gta.estratificacao_gta AS gem ON gem.id_gta = gt.id_gta
                             INNER JOIN dsa.estratificacao AS es ON gem.id_estratificacao = es.id
                             INNER JOIN dsa.especie AS esp ON esp.id = es.id_especie
                    WHERE gt.id_destinovinculado = ac.id_aglomeracaociclo
                      AND gt.tp_destino = 'aglomeracao'
                      AND esp.id_grupo = 1 --Bovídeos
                      AND gta.bo_ativo = TRUE
         ...
         LEFT JOIN (SELECT ac.id_aglomeracaociclo              AS id,
                    ...
                    WHERE gt.id_destinovinculado = ac.id_aglomeracaociclo
                      AND gt.tp_destino = 'aglomeracao'
                      AND esp.id_grupo = 10 --CAPRINO/OVINO
                      AND gta.bo_ativo = TRUE
